...
try:
    ...
    # Récupérer la série pibpHab_lag # 
    if 'PibpHab_lag' in df.columns:
        series = df['PibpHab_lag']
    else:
    ...
    ax1.set_title('Fonction d\'Autocorrélation (ACF) - PibpHab_lag', fontsize=14)
    ...
    ax2.set_title('Fonction d\'Autocorrélation Partielle (PACF) - PibpHab_lag   ', fontsize=14)
    ax2.set_ylabel('Corrélation Partielle')
    ax2.set_xlabel('Décalages (Lags)')
    
    plt.tight_layout()
    plt.show()
    plt.savefig('PyGraphs/ACF_PACF_PibpHab_lag.pdf')
    
    # Affichage de la série temporelle
    plt.figure(figsize=(12, 6))
    plt.plot(series.index, series.values, marker='o', linewidth=2, markersize=4)
    plt.title('Série Temporelle - PibpHab_lag', fontsize=14)
    plt.xlabel('Temps')
    plt.ylabel('PibpHab_lag')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.show()
    plt.savefig('PyGraphs/PibpHab_lag.pdf')
    ...
    print("2. Vérifiez que la colonne 'PibpHab_lag' existe dans le fichier")
