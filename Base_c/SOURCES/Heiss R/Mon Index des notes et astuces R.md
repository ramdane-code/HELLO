...
- Créer un pdata.frame équilibré (n,T) [[Heiss 13 et 14. Data en coupe transversales poolées et Panels_Fr#^fcf4b6]]
- Afficher nbre choisi d'obs de col. choisie[[Heiss 13 et 14. Data en coupe transversales poolées et Panels_Fr#^obsETcol]]
- Calcul sur  variables de panel (et print "collectif") : lag(x), diff(x), between(x), within(x) [[Heiss 13 et 14. Data en coupe transversales poolées et Panels_Fr#^8b2fb9]]
- Paramètres pour semi log : si log(wage)$\sim$ F...  donne Coef= -0,37 cela signifie que wage_F = `exp(-0.37)-1 ≈ -27.1%` de moins que wage_H (il s'agit d'une dummy var) [[Heiss 13 et 14. Data en coupe transversales poolées et Panels_Fr#^66e961]]
# Heiss 15. Variables instrumentales
- Estimation de $β_1$ par OLS :  $\hat{\beta}_1^{OLS} = \frac{\text{Cov}(x,y)}{\text{Var}(x)}$ et par IV : $\hat{\beta}_1^{IV} = \frac{\text{Cov}(z, y)}{\text{Cov}(z, x)}. \tag{15.2}$ [[Heiss 15. Variables instrumentales#^521849]]
- Subset : extraire d'une var (col wage), une var sans obs manquantes (col "oursample") : oursample <- subset(mroz, !is.na(wage)) [[Heiss 15. Variables instrumentales#^54be7e]]
